...
LRS_merged_modified.gtf<-LRS_merged.gtf %>%
  mutate(V2=str_replace(LRS_merged.gtf$V2,"PBRI", "PacBio"))%>%
  mutate(V9=gsub('"G', '"PB.', LRS_merged.gtf$V9)) 
write.table(LRS_merged_modified.gtf,"/../MERGED_TRANSCRIPTOME/LRS_merged_modified.gtf", sep="\t", quote= FALSE , row.name=FALSE, col.names = FALSE)
