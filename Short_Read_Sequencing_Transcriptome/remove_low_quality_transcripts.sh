...
gffcompare /../../SRS.gtf -r SRS.gtf -R -M -o /../gffcompare/SRS -V
...
gtf <- read.delim("/../../SRS.gtf", header=FALSE)
gtf$ID <- paste(gtf$V9)
...
write.table(gtf.good, "/../../SRS.filtered.gtf", sep="\t", row.names = FALSE, col.names=FALSE, quote=FALSE)
